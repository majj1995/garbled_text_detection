...
def _load_audited_ocr(ocr_results: Path | None, ocr_audit: Path | None) -> dict[str, OcrResult]:
...
def _reviewed_candidates(
    row: dict[str, object],
) -> list[tuple[str, BoundingBox, str, str, str | None]]:
    candidates: list[tuple[str, BoundingBox, str, str, str | None]] = []
    ...
    for character in characters:
        ...
        candidates.append(
            (annotation_id, box, "reviewed", decision.value, str(text) if text else None)
        )
    return candidates
...
def extract_character_crops(
    ...
    *,
    ...
    ocr_by_image = _load_audited_ocr(ocr_results, ocr_audit)
    ...
    for row in rows:
        ...
        if image_id in ocr_by_image:
            ocr_candidates = _ocr_candidates(image_id, ocr_by_image[image_id])
        ...
        for source_id, raw_box, annotation_source, decision, text in candidates:
