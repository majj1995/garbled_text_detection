...
已确认的五类笔画规则现已接入正式实验链路：`glyphs generate-v2` →
`train glyph --sampler paired` → `evaluate glyph-v2`。
完整的服务器复制命令见 [V2 生成、训练与评估指南](docs/v2-training.md)。
不再安排额外图片验收；下方预览章节是历史记录，不是当前待办。

V2 将训练、阈值校准、测试分开；同字正常样本成对训练，原型仅来自训练集，
阈值只在校准集正常样本上确定。新样本是规则生成的实验标签，不是逐图人工精标。
源素材仍为 `production_allowed=false`，必须显式允许离线实验，不代表生产授权。
旧数据、预览和模型保留；旧 `glyphs generate` 仍是 V1，不能代替新入口。
...
## 最新规则：加强间隙堵塞，保留其他已确认强度

针对混合批次中 5 张噪点式弱连接，只增强 `block_gap`：连接带加长、加粗，允许越过原笔画两侧；
同时在实际 96px 输入上检查原本空白处真正新增的连续实心区域，排除旧墨迹重叠、浅色边缘、
零散点和细尾巴凑面积/长度的情况。原有局部间隙拓扑、整字变化上限和主体保留检查继续生效。
`close_opening`、添笔、整笔删除、重叠位移和已确认的 2 倍断笔长度均不变。

保留 [5 张原连接位置的新旧对照](docs/previews/glyph-gap-pairs-20260911/index.html)：
“猛”“觅”通过新规则，另 3 个原位置不满足检查而跳过，不换位置补齐。
以及 [20 张新字的间隙堵塞专项预览](docs/previews/glyph-gaps-20260911/index.html)。
用户反馈这 20 张均为明显异常，并确认无需继续验收；当前规则冻结用于新的 V2 离线实验。

归档预览仍为 `REVIEW`、`training_eligible=false`，不回写旧标签。
新 V2 数据单独生成并标明规则标签来源；这不是复杂广告背景或现网误报率评估。

## 保留预览：五类混合复核结果 95 / 5 / 0

打开 [100 张混合预览](docs/previews/glyph-mixed-20260910/index.html)，
先独立判断修改字，再展开原字、完整笔画和变化 mask。
添笔、断笔、粘连、位移、缺笔各 20 张，展示顺序固定打乱。
断笔沿用已获用户强度确认的原始长度 2 倍，横向宽度和保护规则不变；本轮未修改生成算法。

固定种子 `2026091053`，排除此前七个预览档案的全部 178 个来源字，从剩余 3322 字中抽样。
本批 100 张来自 100 个不同汉字，共尝试 112 次、跳过 12 次，五类均无缺额。
粘连实际为错误封口 8 张、间隙堵塞 12 张，未另外设置子型配额；未按外观挑样或换种子重抽。
[抽样及复现说明](docs/previews/glyph-mixed-20260910/reproduce.md) 保存了来源、排除规则和配置。

用户反馈：明显异常 95、合法可接受 5、不确定 0。5 个可接受项均为间隙堵塞：
`sb361978ba729`（猛）、`s58b7f878c5b1`（蠢）、`s453475d07331`（醋）、
`sf7cd087529b7`（派）、`s4ea729dc042e`（觅）。共同问题是连接太短小、像背景噪点。
本节仅记录反馈，归档的候选与审阅模板仍保持原样；没有导入异常训练标签，训练数据和模型未改动。

## 保留预览：原始断笔长度 2 倍的三档同位置对照

打开 [1× / 1.15× / 2× 对照页](docs/previews/glyph-breaks-double-20260910/index.html)。
默认断笔切口长度已从原始版的 1.15 倍调整为 **2 倍**，不是当前版再翻倍得到 2.3 倍。
横向宽度、两段笔画保留与断口净空等保护规则不变，其他四类不变。

保持原来 20 个汉字、同一笔画与切点，2× 有 7 张通过且实际像素变化：寸、姑、剑、查、偎、纹、哮；
另外 13 张未通过保护检查，保留对照位置并标记跳过，没有换字、换笔或换位置补齐。
1× 原始版和已归档的 1.15× 版均保留，便于比较强度；长度倍率不代表像素面积或可见净空的倍率。
用户已确认当前 2 倍切除强度可以；这不是逐图异常标签，归档候选仍为 `REVIEW`、非训练用途。
没有改模型、导入标签或启动实验重训。
[本批复现说明](docs/previews/glyph-breaks-double-20260910/reproduce.md) 记录固定尝试重放与跳过情况。

## 保留预览：断笔切口加长 15% 的同位置对照
...
本轮只调整断笔：根据独立笔画的转折或主体选择位置，不再以两笔接触处或整字开口作为充分条件。
原先连续的一笔必须留下两段有足够面积、长度和原笔画支撑的部分；
合并其他笔画后，断口在实际 96px 输入的前景与核心阈值下仍须清晰可见。
仅截短笔端、分离接头、产生微小碎屑或断口被其他笔画填住的候选不会因此通过。

```bash
uv run poor-word glyphs preview-strokes --breaks-only --per-operator 20 \
  --output-dir artifacts/break-preview
```

`--breaks-only` 只生成断笔，不能与 `--bridges-only` 同时使用；输出目录必须不存在。
数量不足时保留缺额和跳过原因，退出码为 2，不用其他类型补位。
...
## 五类扩样预览：保留的 100 张人工复核基线

打开 [100 张五类扩样预览](docs/previews/glyph-expanded-20260910/index.html)，
先独立判断修改字，再展开原字、完整笔画和变化 mask。
添笔、断笔、粘连、位移、缺笔各 20 张。用户反馈：明显异常 94、合法可接受 6、不确定 0，
可接受项主要出现在断笔的接笔位置；尚未提供逐张编号，因此不推断这 6 张的具体标签映射。

本批固定沿用 `01da55a` 的生成规则，随机种子改为 `2026091017`，
从字池中排除前三批预览已出现的 59 个来源字，没有根据生成后的外观挑选样本。
100 张对应 99 个不同来源字（“落”用于两种不同操作）；粘连中错误封口 11 张、间隙堵塞 9 张。
五类配额完整，共尝试 131 次，31 次因不适用而跳过。
抽样记录与复现命令见 [说明](docs/previews/glyph-expanded-20260910/reproduce.md)。

**全部仍为 `REVIEW`、`training_eligible=false`，未生成训练 manifest，未开始重训。**
本批用于检验生成规则在新汉字上的候选质量，不代表模型准确率或现网效果。
旧预览、其他工程代码和模型保持不变。

## 粘连专项预览：已完成 20 张人工复核

上一轮 50 张人工反馈为明显异常 38、合法可接受 6、不确定 6，可接受项主要是弱粘连。
本轮只调整粘连：**错误封口**和**间隙堵塞**各 10 张，其他四类算子、旧预览及模型不改。
保留 [20 张粘连专项预览](docs/previews/glyph-bridges-20260910/index.html)。
用户反馈这 20 张全部明显异常，合法可接受和不确定均为 0，即两种子型各 10/10。
这只确认该批人工复核结果，不等同于批量候选全部合格。
原始候选文件仍保留生成时的 `REVIEW` 状态，未导入训练标签，暂时不用重新训练。

如需自己生成，在项目根目录执行：

```bash
uv run poor-word glyphs preview-strokes --bridges-only \
  --output-dir artifacts/bridge-preview
```

`--bridges-only` 下，`--per-operator` 表示**每种粘连子型**的数量（默认 10）。
两种配额在生成前固定，不相互补位；某类不足时保留结果和缺额统计，退出码为 2。
`run.json` 记录 `expected_by_bridge_mode`、`by_bridge_mode`、`missing_by_bridge_mode`，
候选及 NPZ 中的 `bridge_mode` 记录实际子型。输出目录必须是新目录。

封口检查原先对外连通的空白是否形成有面积、有宽度的封闭区域；间隙堵塞检查
原图预选的局部空白通路是否被实质性吞并。两类都在实际 96px 输入的宽松前景和
核心前景下验证，连接粗细匹配附近笔画，不再用随机短桥凑数。
首版只处理横、竖方向的口门和间隙；无法确认目标位置的情况跳过，不做斜向结构追踪。
这些几何约束不能证明汉字非法，改成另一个合法字不算异常。
...
重绘剩余笔画，当前断笔已改为单笔内部断裂，位移要求实际 96px 输入出现 15%–45% 的
目标笔画核心重叠且有明显位移。几何门槛不代表汉字合法性判断。
当前代码中的粘连已改为封口/间隙子型，断笔已改为内部断裂；上方旧 50 张附件保留原版本结果，未重新生成。
不适合的操作重试或跳过，不使用 V1/V2 弱扰动兜底。
...
仓库附带 [50 个双字体候选预览](docs/previews/glyph-v2-20260910/index.html)：
拉取代码后，在本地浏览器打开该 HTML 即可，无需 GPU、OCR 或启动服务。
目录中的 `images` 文件夹需与 HTML 一起保留。也可在项目根目录重新生成：

```bash
uv run poor-word glyphs preview-v2 \
  --output-dir artifacts/glyph-v2-preview
```

默认从 3500 常用字中抽取候选，使用仓库附带的 Noto Sans / Noto Serif 两种字体，
添笔、断笔、缺笔、粘连、部件位移各 10 个，每类包含中等、较强两个档位。
如宋体文件缺失，执行 `uv run poor-word data fetch --source-id noto_serif_sc_regular`；
字体来源、SHA-256 与 OFL 许可位于 `data/locks` 和 `data/licenses`。
命令会持续打印候选进度，并拒绝覆盖已有输出目录；重跑时换一个新目录即可。

打开输出目录的 `index.html`，先独立看 96×96 字形，再展开原字、实际改动 mask 和
模型输入通道。按五类分别记录“明显异常 / 合法或可接受 / 不确定或不可读”的数量；
可用 `review-template.jsonl` 保留逐条判断、复核人及理由。如果变成另一个合法汉字，
不算乱码，可在 `confirmed_char` 中记录该字。不确定项保持 `REVIEW`。

实现与边界：

- 根据局部笔画宽度、方向、字形尺寸调整修改尺度，并在 96×96 预处理后检查变化。
  粘连同时校验训练所用前景 mask，避免只在高清图中成立。
- 断笔、缺笔使用局部笔画片段近似，位移使用连通块近似；它们不是完整笔画或偏旁的语义分割。
  强度档位控制采样尺度，重试后落点可能不同，不保证每个字的较强档变化像素严格更多。
- 不满足条件的候选会重试或跳过；`run.json` 记录配额、跳过原因、字体和源码哈希。
  配额未填满时仍保存可审计结果，并以退出码 2 提示，不会强行补入弱扰动样本。
- `candidates.jsonl` 中全部标记 `REVIEW`、`training_eligible=false`。
  几何变化不等于非法汉字，审阅模板也不会自动导入训练；本阶段不据此报告模型效果。
- 当前预览是纯字形校准，不含复杂广告背景；背景鲁棒性留待这一轮人工复核通过后单独验证。

## 生成合成字形

以下 `glyphs generate` 仍是 V1 基线命令。鉴于已发现的合成标签质量问题，
以下保留 V1 复现命令；当前重训请使用文首的 V2 指南，不建议用原配方扩量。
